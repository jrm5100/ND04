...
ggplot(aes(x=EstimatedEffectiveYield, y=InvestmentSpeed),
       data=filter(loans, ListingCreationDate >= ymd('2009-07-01')))+
  geom_point(alpha=1/10)+
  facet_wrap(~ProsperRating, ncol=3)+
  scale_y_log10()+
  geom_smooth(aes(color=Term))+
  ggtitle('Investment Speed vs Estimated Effective Yield across Prosper Ratings')+
  xlab('Estimated Effective Yield')+
  ylab('Investment Speed ($ per hour)')
```

### Description Two
This plot shows the relationship between investment speed (the size of the original loan in dollars divided by the number of hours between listing and origination) and the estimated effective yield, faceted by the Prosper Rating.  Investment speed could be seen as a measurement of lender enthusiasm, although it is possible that other things would affect the time between listing and origination.  The enthusiasm generally seems to increase as the rate increases, but then declines.  This could be because loans with even higher rates have a less acceptable level of risk.  The important result here is that the enthusiasm shows a similar pattern across different Prosper Rating levels even though the estimated yield differs.  For example, a 36-month loan with a 0.1 estimated effective yield and AA rating will be originated as quickly as a similar sized 36-month loan with a .15 yield and C rating.
